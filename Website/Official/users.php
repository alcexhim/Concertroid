<?php
	...
	function printTable($result)
	{
		...
		for ($i = 0; $i < $nRows; $i++)
		{
			...
			echo("<td><a href=\"/u/" . $values["user_id"] . "/\">" . $values["user_displayname"] . "</a></td>");
		...
		if ($path != null)
		{
			...
			else if ($nRows == 1)
			{
				if ($path[1] == "messages" && count($path) == 2)
				{
					...
					echo("<a href=\"/u/" . $path[0] . "/messages/new/\">New Message</a>");
					echo("</td>");
					...
					if ($messageCount > 0)
					{
						...
						for ($i = 0; $i < $messageCount; $i++)
						{
							...
							echo("><a href=\"/u/" . $senderData["user_id"] . "\">" . $senderData["user_displayname"] . "</a></td>");
							echo("<td");
							if ($values["message_status"] == 1)
							{
								echo(" style=\"font-weight: bold;\"");
							}
							echo("><a href=\"/u/" . $path[0] . "/messages/" . $values["message_id"] . "\">" . $values["message_subject"] . "</a></td>");
							echo("<td");
							if ($values["message_status"] == 1)
							{
								echo(" style=\"font-weight: bold;\"");
				...
				else if ($path[1] == "messages" && count($path) > 2)
				{
					...
					if ($messageID == "new")
					{
						echo("<form action=\"/u/" . $path[0] . "/messages/send\" method=\"POST\">");
						echo("<table style=\"width: 100%\">");
						echo("<tr>");
					...
					else
					{
						...
						<table style="width: 100%">
							<tr>
								...
								<td><?php echo("<a href=\"/u/" . $senderData["user_id"] . "\">" . $senderData["user_displayname"] . "</a>"); ?></td>
